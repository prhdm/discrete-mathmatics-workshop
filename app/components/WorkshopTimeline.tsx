...
const DesktopTimeline: FC<{ events: readonly TimelineEvent[] }> = ({ events }) => (
  <div className="hidden sm:block">
    <div className="relative h-28 sm:h-32 -mx-4 sm:-mx-8">
      <div className="absolute top-14 sm:top-16 left-0 w-full h-0.5 sm:h-1 bg-gray-300 dark:bg-gray-600" />
      ...
      {events.map((event, index) => (
        <div key={index} className="relative flex items-center">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2">
            <div className="w-3 h-3 bg-black dark:bg-white rounded-full border-2 border-white dark:border-slate-900 shadow-lg">
...
const TimelineDot: FC<{ event: TimelineEvent; totalEvents: number }> = ({ event, totalEvents }) => (
  <div className="flex justify-center" style={{ width: `${100 / totalEvents}%` }}>
    <div className="w-3 h-3 sm:w-4 sm:h-4 bg-black dark:bg-white rounded-full border-2 sm:border-4 border-white dark:border-slate-900 shadow-lg transform -translate-y-1 sm:-translate-y-1.5">
      <span className="sr-only">{event.title}</span>
    </div>
  </div>
);
